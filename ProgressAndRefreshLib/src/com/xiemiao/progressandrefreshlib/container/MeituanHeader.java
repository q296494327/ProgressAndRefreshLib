...
import com.xiemiao.progressandrefreshlib.utils.DensityUtil;
...
public class MeituanHeader extends BaseHeader {
    ...
    public MeituanHeader(Context context,int[] pullAnimSrcs,int[] refreshAnimSrcs){
        ...
        for (int i=1;i< this.pullAnimSrcs.length;i++) {
            animationPull.addFrame(context.getResources().getDrawable(this.pullAnimSrcs[i]),100);
            animationRefresh.setOneShot(true);
        }
        for (int i= this.pullAnimSrcs.length-1;i>=0;i--){
            animationPullFan.addFrame(context.getResources().getDrawable(this.pullAnimSrcs[i]), 100);
            animationRefresh.setOneShot(true);
        }
        for (int src: this.refreshAnimSrcs) {
            animationRefresh.addFrame(context.getResources().getDrawable(src),150);
